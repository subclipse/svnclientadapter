    	...
    	if (!availabilityCached) {
    		...
    		if (!available) {
    			...
    			try {
    				...
    				if (version.getMajor() == 1 && version.getMinor() == 14)
    					available = true;
    				else {
    					available = false;
    					javaHLErrors = new StringBuffer("Incompatible JavaHL library loaded.  Subversion 1.14.x required.");
    				}
    			} catch (UnsatisfiedLinkError e) {
    				available = false;
    				javaHLErrors = new StringBuffer("Incompatible JavaHL library loaded.  1.14.x required.");
    			}
    		}
    	}

    		...
    		if (!available) {
    			...
    			try {
    				...
    				if (version.getMajor() == 1 && version.getMinor() >= 12)
    					...
    					available = false;
    					javaHLErrors = new StringBuffer("Incompatible JavaHL library loaded.  Subversion 1.12 or higher required.");
    				}
    			} catch (UnsatisfiedLinkError e) {
    				available = false;
    				javaHLErrors = new StringBuffer("Incompatible JavaHL library loaded.  1.12 or higher required.");
    			}
    		}
    	}
